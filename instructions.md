...
14. **useTeam**
    ...
    - **Описание**: Ищет фотографии по заданному запросу. Можно предать диалог как массив строк где последнее от пользователя или оставить пустым - тогда передается текущий диалог
    - **Пример использования**: 
      ```
...
15. **usePhoto**
    - **Синтаксис**: `usePhoto(search, count)`
    - **Описание**: Ищет фотографии по заданному запросу.
    - **Пример использования**: 
      ```
      {{usePhoto('красивый закат', 1)}}
      ```
    - **Результат**: 
     ```
