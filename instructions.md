...
1. **setModel**
   ...
   - **Описание**: Поле `settings` позволяет безопасно изменить параметры `model`, `temperature`, `top_p`, `presence_penalty` модели. Поле `extra` опционально и позволяет настроить модель более тонко, а так же добавить фцнкции выполнения. Смотри [расширенное использование OpenAI API)]([https://developer.mozilla.org/en-US/docs/Web/JavaScript](https://platform.openai.com/docs/api-reference/chat/create#chat-create-tools))
   - **Пример использования**: 
     ```
     {{setModel({top_p: 0.75})}}
     ```
   - **Результат**: 
     Значение `top_p` в настройках модели поменялось на `0.75` 
---
2. **useGpt4**
