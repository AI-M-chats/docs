...
-   **Синтаксис**: `useWeb({url, actions, text})`
    
-   **Описание**: Выполняет серию действий на указанной веб-странице с использованием Puppeteer и Cheerio. Позволяет автоматизировать взаимодействие с веб-контентом, проверяя и обрабатывая элементы на странице.
...
-   **Параметры**:
    ```
    {
       ...
       actions?: [ // Массив действий, которые необходимо выполнить
    	{
    		type: 'wait' | 'click' | 'type' | 'goto' | 'if' | 'while' | 'hover' | 'scroll' | 'keyboardType',
                  selector?: 'CSS-селектор элемента', // Необязательно: элемент для взаимодействия
    		text?: 'поиск элемента по тексту', // Используется вместо селектора
    		input?: 'Текст для ввода', // Необязательно: текст для действий типа 'type'
    		n?: number, // Необязательно: индекс элемента, если селектор подходит для нескольких
    		timeout?: number, // Необязательно: время ожидания в миллисекундах
    		x?: number, // для позиционирования мыши или скрола
    		y?: number, // для позиционирования мыши или скрола
                url?: адрес для перехода в окне
                condition?: массив actions // используется для событий типа if или while
    	 }
       ], 
       text?: 'true если нужно трансформировать html в текстовое представление'
       cond?: 'Условие выполнения',
       condjs?: 'Функция js должна возвращать boolean',
       ctx?: 'Имя, если передано, то результат сохраняется в контекст под этим именем'
    }
...
-   **Пример использования**:
    ...
    actions: [
        ...
        {
            type: 'wait',
            selector: 'h1', // Ожидание заголовка на странице
            timeout: 2000 // Ожидание 2 секунды
        },

        // 2. Клик по элементу
        {
            type: 'click',
            selector: '.btn', // Клик по кнопке с классом btn
            n: 0 // Индекс первого элемента
        },

        // 3. Ввод текста в элемент
        {
            type: 'type',
            selector: '#input-field', // Ввод текста в поле с id input-field
            input: 'Hello, World!'
        },
        ...
        {
            type: 'goto',
            url: 'https://another-example.com' // Переход к новому URL
        },

        // 5. Условное выполнение
        {
            type: 'if',
            selector: '.optional-element', // Если элемент с классом optional-element присутствует
            condition: [
                {
                    type: 'click',
                    selector: '.confirm-button' // Клик по кнопке подтверждения
                }
            ]
        },
        ...
        {
            type: 'while',
            selector: '.loading', // Проверка существования элемента loading
            condition: [
                {
                    type: 'wait',
                    timeout: 1000 // Ожидание 1 секунды между проверками
                }
            ]
        },

        // 7. Наведение на элемент
        {
            type: 'hover',
            selector: '.dropdown' // Наведение курсора на элемент с классом dropdown
        },

        // 8. Прокрутка вниз
        {
            type: 'scroll',
            x: 0, // Прокрутка по оси X (не используется)
            y: 500 // Прокрутка вниз на 500 пикселей
        },

        // 9. Ввод текста с клавиатуры
        {
            type: 'keyboardType',
            input: 'This is typed using keyboard.' // Ввод текста через клавиатуру
        }
    ],
    text: true // Преобразовать результат в текст
    })}}		
   	``` 
    
-   **Результат**:
    `Возвращает текстовое содержимое страницы, если 'text' установлен в true или html иначе`
---
23. ### useKeyManager
...
- Синтаксис: `setCtx({name})`
- Описание: Функция setCtx сохраняет контекст. 

- Параметры:
```
  {
	[key]: 'какие js даные нужно использовать по key'
	cond?: 'Условие выполнения',
	condjs?: 'Функция js должна возвращать boolean',
	ctx?: 'Имя, если передано, то результат сохраняется в контекст под этим именем'
  }
...
- Пример использования: `{{setCtx({pets: 'dog, cat' })}}`

- Результат: в контексте появиться переменая `pets` в которой будет содержаться строка 'dog, cat'.
